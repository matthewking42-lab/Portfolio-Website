...
function Stat({ value, label, sub }: { value: string; label: string; sub?: string }) {
  return (
    <div className="flex flex-col gap-1 px-8 py-6 bg-white">
      ...
      <span className="font-mono text-xs uppercase tracking-widest text-gray-400">{label}</span>
        ...
        <div className="w-full grid md:grid-cols-2 gap-12 lg:gap-20 items-center py-16 md:py-20">
          ...
          <div style={{ animation: 'fade-up 0.7s ease-out 0.1s both' }}>
            ...
            <p className="text-gray-500 text-base md:text-lg leading-relaxed max-w-lg mb-10">
              Designing railway bridges and structural infrastructure across the UK,
              with a focus on precision, safety, and longevity.
            </p>
      ...
      <section className="border-t border-gray-100">
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-3 divide-x divide-gray-100">
            <Stat value={String(projectCount)} label="Projects" sub="Across the UK" />
            <Stat value="Rail" label="Primary sector" sub="Network Rail" />
            <Stat value="2018" label="Career start" sub="Trainee → Design Engineer" />
          </div>
        </div>
      </section>
    </main>
